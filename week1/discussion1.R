...
janitor::clean_names() |>  # update column headers to snake case "_"
  ...
  # clean up dates ----
mutate(job_start_date = str_remove(job_start_date, " AM")) |> # remove 'AM' from the job_start_date string 
  mutate(datetime_start = mdy_hms(job_start_date)) |> # convert the job_start_date from a string to a datetime_start as a new columns.
  mutate(year = year(datetime_start)) |> # create a new year column with years taken from datetime_start column
  
  # select relevant cols ----
select(datetime_start, year, state_name, county_name, well_name, total_base_water_volume) |> # selects columns that will be used and leaves out non relevant columns
  
  # filter out non-state names ----
filter(!state_name %in% c("Beaver", "Beckham", "Harper", "Hemphill", "Midland", "Red River", "Roosevelt", "Rusk", "State", "WARD")) |> # filters out objects in the state_name column that are not state names
  
  # renames state_name columns to "sn" because it is shorter and is easier when using case_when (not necessary) ----
rename(sn = state_name) |> 
  
  # make all words title case ----
mutate(sn = str_to_title(sn)) |> # uses the mutate function to change all objects in the sn(state name) column to title case
  
  # fix misspelled state names ----
mutate(sn = case_when(   # uses the mutate function to manually fix objects with spelling errors in the sn (state name) column.
  ...
  # filters out rows that contain a '?' mark as an object----
filter(!str_detect(string = sn, pattern = "\\?")) |> # `?` is a special chr; escape with `\\` prefix
  
  # make all objects in the sn column uppercase in order to covert the objects to an abbreviation of state names ----
